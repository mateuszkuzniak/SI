...
[ --- \ ]% <separator>
[]% <end>
\setcounter{tocdepth}{1}% Display up to \subsection in ToC
...
\subsection*{Informacja uzupełniająca}

W połowie maja bieżącego roku temat pracy semestralnej naszej grupy uległ zmianie z powodu niespodziewanej
złożoności poprzedniego zadania (łamigłówka \textit{BagPuzzle} jako problem CSP).
Z tego powodu nasza grupa została zwolniona z jednych zajęć laboratoryjnych 
(nie oddano wtedy sprawozdania roboczego).

Na kolejnych zajęciach, tj. 17 czerwca, zostało zaprezentowane wstępne sprawozdanie końcowe dotyczące
nowego już tematu. Pełni ono rolę sprawozdania roboczego z tychże zajęć.
...
\subsubsection*{Biblioteka \texttt{minizinc} dla języka \textit{Python}}

W naszym programie wykorzystujemy bibliotekę w wersji \texttt{0.4.2}. Zauważyliśmy, że posiada ona API
(ang. \textit{Application Programming Interface}), które nie jest jeszcze w pełni zaimplementowane przez
twórców biblioteki.

Jako przykład możemy podać próbę wprowadzenia możliwości wyświetlania wszystkich rozwiązań problemu do naszej aplikacji.
Metoda instancji problemu o nazwie \texttt{solve} posiada parametr \texttt{all\_solutions} typu logicznego, który w zamyśle
ma spowodować zwrócenie wszystkich rozwiązań - w naszym przypadku tych, które mają maksymalną liczbę punktów.

Przypisanie parametrowi \texttt{all\_solutions} wartości \texttt{True} powoduje wystąpienie w programie błędu
\texttt{NotImplementedError}, który informuje o braku tej funkcjonalności w bibliotece.

Bardzo możliwe że inne części tej biblioteki również nie są w pełni zaimplementowane. W naszej aplikacji
wykorzystaliśmy funkcjonalności dostępne w wersji \texttt{0.4.2}.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{ | p{\dimexpr.28\linewidth-2\tabcolsep-1.3333\arrayrulewidth} | X | }
        ...
        \begin{itemize}
            ...
            \item implementacja panelu planszy w aplikacji
            \item implementacja odczytu i zapisu plansz do pliku
        \end{itemize} \\
        \hline
        Hubert Knioła    &
        \begin{itemize}
            \item pomoc merytoryczna przy sprawozdaniu
            \item projekt menu aplikacji
            \item implementacja panelu podsumowania rozwiązania
        \end{itemize} \\
        \hline
        Mateusz Kuźniak  &
        \begin{itemize}
...
Kod źródłowy projektu można znaleźć wewnątrz folderu \texttt{src} w głównym
katalogu repozytorium \cite{repository}. Zrealizowany program zawiera w sobie 8 klas z czego 3 są elementami czysto prezentacyjnymi - mowa
...
projektu \cite{repository}. W pliku \texttt{requirements.txt} można znaleźć wszystkie wykorzystywane biblioteki wraz z ich wersjami.
...
\textbf{,,Save current canvas to file''}. Spowoduje to otwarcie dialogu omawianego w paragrafie \textit{\nameref{sec:zpliku}}.
...
instrukcją zawartą w paragrafie \textit{\nameref{sec:instrukcja}}.
...
\section{Tekst programu}

Pełen kod rozwiązania w języku \textit{MiniZinc} można zobaczyć na listingu 1.

\hfill

\lstinputlisting[numbers=left, captionpos=b, frame=single, language=Python, caption=Rozwiązanie łamigłówki w języku \textit{MiniZinc}]{./nocommentrogo.mzn}
